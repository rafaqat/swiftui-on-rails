...
class SwiftUIReactiveTransport {
  ...
  handleStateMutations(mutations) {
    ...
    mutations.forEach(mutation => {
      ...
      // A malformed data-*-changes attribute on one mutation must not abort
      // processing of the remaining mutations in this batch. Guard per
      // mutation, mirroring setupObservers's parse handling.
      try {
        // Parse state changes
...
class InteractionBehavior {
  ...
  connect() {
    ...
    if (keypress?.scope === 'window') {
      // Resolve the event name once at connect time. disconnect() must remove
      // the exact listener we added even if data-sui-keypress (or its phase)
      // is mutated while connected, otherwise the listener leaks.
      this.globalKeyEventName = keypress.phase === 'keyup' ? 'keyup' : 'keydown'
      this.globalKeyHandler = (event) => this.filterKeyPress(event)
      document.addEventListener(this.globalKeyEventName, this.globalKeyHandler)
    }
  ...
  disconnect() {
    ...
    if (this.globalKeyHandler) {
      document.removeEventListener(this.globalKeyEventName, this.globalKeyHandler)
      this.globalKeyHandler = null
      this.globalKeyEventName = null
    }
  }
