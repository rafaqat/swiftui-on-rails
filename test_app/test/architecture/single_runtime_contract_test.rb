...
class SingleRuntimeContractTest < ActiveSupport::TestCase
  ...
  test "Stimulus is absent from dependencies imports and generated applications" do
    ...
    # The gem-root Gemfile.lock is generated by `bundle install` and not
    # committed, so it is absent in a fresh CI checkout. Check every path that
    # actually exists — a file that isn't there can't reference Stimulus, and
    # the committed dependency manifests (gemspec, test_app Gemfile/lock,
    # importmap, application.js, install generator) are always present.
    present = paths.select(&:exist?)
    assert_operator present.length, :>=, paths.length - 1,
      "expected at most the generated root Gemfile.lock to be absent, missing: " \
      "#{(paths - present).map { |p| p.relative_path_from(PROJECT_ROOT) }.join(', ')}"
